---
title: Docs Layout Test Page
description: A comprehensive test page covering all docs layout features, shortcodes, and markdown elements.
toc: true
added:
  show_badge: true
  version: 0.1.0
aliases:
  - '/test-category'
  - '[[docsref:/test-page]]'
---

export const bgColor = '#888'
export const fgColor = '#fff'

## Overview

This page exists to test the docs layout and all available content features. It covers frontmatter options, markdown elements, all shortcode components, code blocks, tables, and callouts.

Test:  
asdasda

---

## Badges

<AddedIn version="1.2.0" /> <DeprecatedIn version="2.0.0" />
<CSSOnly tooltip="This feature is CSS only" />
<InFigma tooltip="This feature is available in Figma" />

## Blockquote

<Blockquote>Good design is as little design as possible.</Blockquote>

<Blockquote attr="Charles Eames">
  The details are not the details. They make the design.
</Blockquote>

## Callouts

<Callout type="info">
  This is an **info** callout with `inline code` and [link to
  docs]([[docsref:/test-category/test-page]]). Use it for general notes and
  tips.
</Callout>

<Callout type="warning">
  This is a **warning** callout with `inline code` and [link to
  docs]([[docsref:/test-category/test-page]]). Use it to highlight something
  important.
</Callout>

<Callout type="danger">
  This is a **danger** callout with `inline code` and [link to
  docs]([[docsref:/test-category/test-page]]). Use it for breaking changes or
  destructive actions.
</Callout>

## Code Blocks

Syntax-highlighted code blocks can be added using standard markdown fenced code blocks with language identifiers, or the `<Code>` shortcode for explicit control over formatting and display options.

### Markdown code

```html
<!-- A code block with syntax highlighting for HTML -->
<div class="container">
  <h1 class="font-5xl">Hello, Chassis</h1>
  <p class="fg-subtle">Token-powered UI system.</p>
</div>
```

```scss
// A code block with syntax highlighting for SCSS
.my-component {
  color: var(--cx-fg-main);
  background-color: var(--cx-bg-even);
  border: 1px solid var(--cx-border-subtle);
}
```

```ts
// A code block with syntax highlighting for TypeScript
import { getConfig } from '@libs/config'

const config = getConfig()
console.log(config.title)
```

```sh
# A code block with syntax highlighting for shell commands
pnpm install && pnpm dev
```

### Code shortcode

<Code lang="sh" code="pnpm install && pnpm dev" />

<Code
  lang="json"
  code={`{
    "chassis": {
      "build": {
        "brands": ["chassis", "example"],
        "apps": { "docs": ["web"] }
      }
    }
  }`} />

## Examples

Live preview with markup using the `<Example>` shortcode.

### With markup

<Example
  code={`<button type="button" class="button primary">Primary</button>
  <button type="button" class="button default">Default</button>
  <button type="button" class="button danger">Danger</button>`} />

### Preview only

<Example
  showMarkup={false}
  code={`<button type="button" class="button primary">Primary</button>
  <button type="button" class="button default">Default</button>
  <button type="button" class="button danger">Danger</button>`} />

### With external files

<ScssDocs name="example-scss" file="content/docs/test-category/example.scss" />
<JsDocs name="example-js" file="content/docs/test-category/example.js" />

## Placeholders

<Example code={`<Placeholder class="image fluid mb-md w-100" height="160" text="SVG Element" />
<Placeholder markup="img" class="image w-100 object-fit-cover mb-md" height="160" text="IMG Element" background=${bgColor} color=${fgColor} />`} />

## Icons

<Example code={`<Icon name="info-circle-solid" />
  <Icon name="info-circle-solid" class="icon-secondary" size="48" /> 
  <Icon name="info-circle-solid" class="icon-reset" font />`} />

## Tables

<CxTable class="table table-striped">
| Variable | Default | Description |
| ---------|-------- | ------------|
|`--cx-fg-main` | `#1a1a1a` | Primary foreground color |
| `--cx-bg-main` | `#ffffff` | Primary background color |
| `--cx-border-subtle` | `rgba(0,0,0,.1)` | Subtle border color |
| `--cx-primary` | `#0066ff` | Brand primary color |
</CxTable>

## Javascript

<Example lang="html" code={`<button type="button" class="button primary" data-cx-toggle="dialog" data-cx-target="#exampleModalLive">Show modal</button>
<a href="#exampleModalLive" class="button link" data-cx-toggle="dialog">Show modal</a>

  <dialog class="modal dialog" id="exampleModalLive" aria-labelledby="exampleModalLiveLabel">
    <div class="modal-header">
      <h2 class="modal-title" id="exampleModalLiveLabel">Modal title</h2>
      <button type="button" class="close-button" data-cx-dismiss="dialog" aria-label="Close"></button>
    </div>
    <div class="modal-body">
      <p>Modal body content goes here.</p>
    </div>
    <div class="modal-footer">
      <button type="button" class="button primary">Take action</button>
      <button type="button" class="button default" data-cx-dismiss="dialog">Close</button>
    </div>
  </dialog>`} />
